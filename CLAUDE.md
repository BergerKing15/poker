...
- `config.BOT_SIMULATIONS` (300) is the whole runtime cost of a bot: pre-flop is a table
  lookup, so this only buys post-flop precision. It is set to the most precision that
  still sustains 250+ hands/min six-handed - 325 is the highest that cleared it on every
  run but only by 9%, while 300 keeps a 31% margin, and this is single-threaded so a
  slower machine lands below these figures. Raising it costs throughput and nothing else;
  lowering it makes the bots measurably worse (RMS equity error 0.025 at 300, 0.041 at
  100, 0.085 at 25, against a 20,000-simulation reference).
